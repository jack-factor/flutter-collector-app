...
Future<Database> database() async {
  ...
  return openDatabase(
    ...
    onCreate: (db, version) {
      ...
      final result = db.execute('''CREATE TABLE comics_marvel(
      ...
      data.forEach((element) { 
        ...
        element['title'] = title.replaceAll(" ${result[result.length - 1]}", "");
...
Future <ComicTotalModel> getTotalRows() async {
  final db = await database();
  final data = await db.rawQuery('SELECT  count(id)  as total,  is_exist FROM  comics_marvel GROUP BY  is_exist');
  await db.close();
...
Future <List <ComicModel>> getAll(int isExist) async {
  ...
  await db.close();
  return data.isNotEmpty ? data.map((s) => ComicModel.fromJson(s)).toList(): [];
}

Future <ComicModel?> getDetail(int pk) async {
  final db = await database();
  final data = await db.query('comics_marvel', where: 'id=?', whereArgs: [pk]);
  await db.close();
...
Future setExist(ComicModel comic) async {
  ...
  await db.update('comics_marvel', comic.toJson(), where: 'id=?', whereArgs: [comic.id]);
  await db.close();
}